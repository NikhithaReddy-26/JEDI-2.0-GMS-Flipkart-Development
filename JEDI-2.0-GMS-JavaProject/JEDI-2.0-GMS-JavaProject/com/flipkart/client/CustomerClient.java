...
//        Check if credentials are right
    ...
    private void bookSlotSubMenu(String userName,int userId){
        ...
        if(centreListByLocation.isEmpty()){
            ...
            bookSlotSubMenu(userName,userId);
            return;
        }
        System.out.print("Choose a gymCentre ID to proceed:");
        int chosenGym = scanner.nextInt();
        //Select Date
        Date sqlDate = selectDate();
        //Choose Slot
        chooseSlot(chosenGym,userName,sqlDate,chosenGym,userId);
    }
...
//            }
    ...
    private void chooseSlot(int gymCentreId, String userName, Date sqlDate, int centreId,int userId){
        ...
        //Select Slot to book
        if(!customerService.bookSlot(userName,sqlDate,slotID,centreId,userId))
        {
            chooseSlot(gymCentreId, userName, sqlDate,centreId,userId);
        }
    }

    private void printUserPlan(String userName,int userId){
        System.out.println("Bookings : ");
        List<UserPlan> allUserPlan= customerService.getCustomerPlan(userId);
        List<Booking> bookingList = customerService.getCustomerBookings(userId);
        System.out.println(DASHED_LINE);
    ...
    private void printbookingsSubMenu(int userName){
        System.out.println("Bookings : ");
    ...
    private void cancelBookingSubMenu(String userName,int userId){
    ...
    public void customerClientMainPage(String userName) {
        ...
        int userId = customerService.getUserId(userName);
        while(true){
            ...
            switch(choice){
                ...
                case 2:
                    ...
                    break;
                case 3:
                    printUserPlan(userName,userId);
                    break;
                case 4:
                    cancelBookingSubMenu(userName,userId);
                    break;
                case 5:
