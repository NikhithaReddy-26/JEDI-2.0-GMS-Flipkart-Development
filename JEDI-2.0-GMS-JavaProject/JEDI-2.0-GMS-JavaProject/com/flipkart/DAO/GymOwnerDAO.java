...
public class GymOwnerDAO implements GymOwnerInterfaceDAO{
    ...
    public int registerGymOwner(GymOwner gymOwner){
        try{
            ...
            statement.setString(2,gymOwner.getEmail());
            statement.setString(3,gymOwner.getPassword());
            statement.setString(4,gymOwner.getPanNumber());
            statement.setString(5,gymOwner.getCardDetails());
            statement.setInt(6,0);

            statement.executeUpdate();
            System.out.println("Registration Success\n");
            return loginGymOwner(gymOwner.getUserName(),gymOwner.getPassword());
    ...
    public List<GymOwner> getPendingGymOwnerList() {
        ...
        try {
            ...
            ResultSet rs = statement.executeQuery();
            while(rs.next()) {
                GymOwner owner = new GymOwner(rs.getString("userName"), rs.getString("email"), rs.getString("password"), rs.getString("panNumber"), rs.getString("cardDetails"));
    ...
    public boolean checkOwnerApproved(int gymOwnerId) {
        try {
            conn = DBConnection.connect();

            statement = conn.prepareStatement(SQLConstants.CHECK_OWNER_APPROVAL);
            statement.setInt(1,gymOwnerId);
            ResultSet rs = statement.executeQuery();
            while(rs.next())
            {
