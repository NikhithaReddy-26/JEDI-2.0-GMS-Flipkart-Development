...
public void registerCustomer(String userName, String password, String email, String phoneNumber, String cardNumber) throws RegistrationFailedException {
    try {
        ...
        if (rowsAffected > 0) {
            ...
            throw new RegistrationFailedException("Failed to register the user. No rows affected.");
        }
    } catch (SQLException exp) {
        throw new RegistrationFailedException("Failed to register the user. Try again. Error: " + exp.getMessage());
    } catch (Exception e) {
        System.out.println("Oops! An error occurred. Try again later. Error: " + e.getMessage());
        e.printStackTrace();
    }
}
